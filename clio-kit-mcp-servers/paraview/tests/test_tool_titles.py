"""Tests asserting every ParaView MCP tool exposes a compact, non-empty title.
...
try:
    ...
    IMPORT_ERROR = None
except ImportError as e:  # pragma: no cover - environment without ParaView deps
    mcp = None
    IMPORT_ERROR = e
...
async def test_expected_tool_count_has_titles() -> None:
    """The full curated set of 26 ParaView tools should all carry titles."""
    if mcp is None:
        pytest.skip(f"ParaView MCP server not importable: {IMPORT_ERROR}")
