...
"""
    ...
    def test_job_submission_latency(self, quick_script):
        ...
        assert avg_latency < 2.0, (
            f"Average submission latency too high: {avg_latency:.3f}s"
        )
        assert max_latency < 6.0, (
            f"Maximum submission latency too high: {max_latency:.3f}s"
        )
    ...
    def test_job_status_query_performance(self, quick_script):
        ...
        try:
            ...
            assert avg_latency < 1.0, (
                f"Status query latency too high: {avg_latency:.3f}s"
            )
            assert max_latency < 3.0, (
                f"Max status query latency too high: {max_latency:.3f}s"
            )
    ...
    def test_concurrent_job_submissions(self, quick_script):
        ...
        # Performance requirements
        assert successful_submissions >= num_concurrent * 0.8, (
            "Too many failed submissions"
        )
        assert total_time < 10.0, (
            f"Concurrent submissions took too long: {total_time:.3f}s"
        )
...
"""
        ...
        try:
            ...
            result = submit_array_job(
                script_path, array_range="1-3", cores=1, job_name="array_perf_test"
            )
            end_time = time.time()
            ...
            assert submission_time < 5.0, (
                f"Array job submission too slow: {submission_time:.3f}s"
            )
    ...
    def test_sustained_load_performance(self, quick_script):
        ...
        while time.time() - start_time < duration_seconds:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = [
                    executor.submit(submit_and_track) for _ in range(max_concurrent)
                ]
        ...
        assert total_submissions > 5, (
            f"Too few successful submissions: {total_submissions}"
        )
