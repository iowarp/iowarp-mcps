"""GeoJSON inspection helpers built on the Python standard library only.

This module reads, validates, and summarizes GeoJSON using nothing but ``json``
and ``math`` — no geopandas/shapely. It is intentionally domain-neutral: it
makes no assumptions about what the features represent, only about GeoJSON
structure as defined by RFC 7946.

All functions accept GeoJSON as either a filesystem path to a ``.geojson``/JSON
file or inline GeoJSON (a JSON string, a parsed ``dict``/``list``). The four
public entry points (:func:`inspect_geojson`, :func:`validate_geojson`,
:func:`summarize_geojson`, and :func:`feature_bbox`) all share the same loader
and the same bounding-box helper so their results stay consistent.
"""
