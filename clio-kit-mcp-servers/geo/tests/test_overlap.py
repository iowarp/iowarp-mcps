...
POLY = {"type": "FeatureCollection", "features": [{
    "type": "Feature", "properties": {"smoke": "3-25"},
    "geometry": {"type": "Polygon", "coordinates": [[[-118.5, 34.0], [-118.0, 34.0],
...
def test_buffer_includes_near_points():
    near = {"type": "FeatureCollection", "features": [_pt(-117.95, 34.2, name="near")]}  # ~4-5km east of edge
    assert points_in_polygons(near, POLY)["matched_count"] == 0
    assert points_in_polygons(near, POLY, buffer_km=15)["matched_count"] == 1


def test_empty_inputs():
    assert points_in_polygons({"type": "FeatureCollection", "features": []}, POLY)["matched_count"] == 0
    assert points_in_polygons({"type": "FeatureCollection", "features": [_pt(-118.2, 34.2)]},
                              {"type": "FeatureCollection", "features": []})["matched_count"] == 0


def test_accepts_path(tmp_path):
    import json
    pp = tmp_path / "pts.geojson"; pp.write_text(json.dumps({"type": "FeatureCollection", "features": [_pt(-118.2, 34.2, name="in")]}))
    gp = tmp_path / "poly.geojson"; gp.write_text(json.dumps(POLY))
