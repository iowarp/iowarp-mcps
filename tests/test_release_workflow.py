...
def test_ares_probe_exercises_persistent_uv_tool_installation() -> None:
    ...
    assert probe_module["EXPECTED_JARVIS_VERSION"] == "1.3.16"
...
def test_wheel_smoke_binds_jarvis_artifacts_to_exact_release_wheel() -> None:
    ...
    match = re.fullmatch(
        r"jarvis-cd @ "
        r"(https://github\.com/grc-iit/jarvis-cd/releases/download/v1\.3\.16/"
        r"jarvis_cd-1\.3\.16-py3-none-any\.whl)"
        r"#sha256=([0-9a-f]{64})",
        dependency,
    )
    ...
    assert jarvis_package["version"] == "1.3.16"
    ...
    assert 'installed.version == "1.3.16"' in smoke_block
    ...
    assert 'jarvis_package["version"] == "1.3.16"' in smoke_block
